...
|------|-------|----------------------|------|
| SETTLE-R04 | The three mappings have `validate="false"` and `SettlementForm.validate` returns nothing, so **no recorded input ever produces a validation error**. `validationErrors` is `[]` for all six transcripts, for save and for detail; the one exception is the CHG-001 calculate check (SETTLE-R18 v2, below). | `api/ScreenResponse.of` | `SettlementControllerTest.settleR04R05NoParametersNoValidationErrors` |
...
| SETTLE-R18 (save route) | A non-blank, non-numeric `deductible` (`abc`) has **no fallback**: `Double.parseDouble` throws and the global exception handler renders `error.jsp` with **HTTP 200**, not 4xx/5xx. Still reproduced on **save**, which `docs/changes/CHG-001-invalid-deductible.md` leaves out of scope (OQ-15 a). | `SettlementCalculator`, `LegacyErrorHandler.numberFormat` | `SettlementCalculatorTest.settleR18NonNumericDeductibleThrows`, `SettlementControllerTest.settleR18SaveNonNumericDeductibleIsErrorScreenWith200` |
| SETTLE-R18 v2 (calculate route) | **Changed, approved.** On calculate the service diverges from the monolith: a non-blank, non-numeric `deductible` is rejected before any calculation and the calculate screen is redisplayed with HTTP 200, `validationErrors = ["settlement.deductible.invalid"]`, empty `fields` (OQ-15 c, decided: empty) and no result, so no `settlementAmount` is computed. This is CHG-001 approved future-state behaviour, not an accidental divergence: see `docs/changes/CHG-001-invalid-deductible.md` and SPEC-SETTLE-001 v0.3. Absent, empty and whitespace-only deductibles are unchanged (R16, R17). No recorded transcript sends an invalid deductible, so parity is unaffected. | `LegacyCoercions.isInvalidDeductible`, `SettlementController.calculate`, `ScreenResponse.invalid` | `LegacyCoercionsTest.settleR18V2InvalidDeductibleIsNonBlankAndUnparseable`, `SettlementControllerTest.settleR18V2InvalidDeductibleRedisplaysCalculateWithValidationError`, `settleR18V2InvalidDeductibleOnGetIsRejectedToo`, `settleR16R17BlankDeductibleIsNotRejectedByR18V2` |
...
|------|------------------|-------------------|-----|
| SETTLE-R18 -> SETTLE-R18 v2 | `POST /settlement/calculate.do` with `deductible=abc` renders `error.jsp` (HTTP 200) from `Double.parseDouble`. | `POST /api/settlement/calculate` returns HTTP 200, `legacyForward = /WEB-INF/jsp/settlement/calculate.jsp`, `validationErrors = ["settlement.deductible.invalid"]`, empty `fields`, `data = null`; the calculator is never invoked. | Approved change record `docs/changes/CHG-001-invalid-deductible.md` (SPEC-SETTLE-001 v0.3, rule SETTLE-R18 v2). Save keeps SETTLE-R18; OQ-15 stays open except (c), decided as empty fields. |
