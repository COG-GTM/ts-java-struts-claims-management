...
| QUIRK-08 | Amounts are parsed with `Double.parseDouble`, so `1d`, `0x1p1`, `Infinity`, `NaN` and surrounding whitespace are accepted and there is no length limit on the input. A first service draft parsed with `BigDecimal` and would have rejected these; review of the legacy source caught it. | `ClaimsActionSupport.decimal`, `SettlementCalculator.calculate` | pull request review against the legacy source | `LegacyCoercions` (`Double.parseDouble`), `LegacyCoercionsTest.acceptsEveryLegacyDoubleForm` | SETTLE-R03, SETTLE-R04, SETTLE-R06 |
