...
| SETTLE-R40 | The saved row records `calculated_by` as the session `user` attribute, which the transcripts show as `supervisor`.                                          | `settlement_save` (`savedBy: supervisor`); `SSA` 36-37                                             | Observed |
...
| SETTLE-R45 | The detail screen shows the settlement with the highest `settlement_id` for the claim, which equals the most recent save only while every row is allocated by `SettlementSaveAction` (R39), since `SettlementService` allocates `claimId + 10000` instead (R47). | `SettlementDAO` 100-109 (`order by settlement_id desc`, first row); `SDA` 18; `SettlementService` 25 | Inferred |
