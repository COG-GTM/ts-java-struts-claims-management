...
| ID       | SPEC-SETTLE-001                                                       |
| Version  | 0.1                                                                   |
| Status   | Draft, derived from observed behaviour only                           |
...
| SETTLE-R02 | A POST to `/settlement/save.do` recomputes the settlement from the request parameters, persists it, and forwards to `/WEB-INF/jsp/settlement/save.jsp` with HTTP 200. | `settlement_save` (`result`, `status`); `struts-config` 214-222; `SSA` 30-45                     | Observed |
| SETTLE-R03 | A request to `/settlement/detail.do` loads the most recently saved settlement for the claim and forwards to `/WEB-INF/jsp/settlement/detail.jsp`.         | `struts-config` 356-363; `SDA` 17-23; `SettlementDAO` 100-115                                     | Inferred |
...
| SETTLE-R06 | The `policyLimit` value submitted by the browser is ignored; the limit always comes from the claim's policy (or the fallback in SETTLE-R14).                  | `SettlementForm` 16, 53-61 (declared) versus `SCA` 25-31 and `SSA` 26, 33 (never read)             | Inferred |
...
| ID         | Rule                                                                                                                                                       | Evidence                                                                                         | Status   |
|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------|----------|
| SETTLE-R37 | Saving does not reuse a previously calculated result; the save action recomputes the settlement from the request parameters using the same calculator as calculate. | `settlement_save` (same inputs as `settlement_calculate` give the same 1000.00); `SSA` 30-33        | Observed |
...
| SETTLE-R40 | The saved row records `calculated_by` as `String.valueOf(session.user)`, which becomes the literal string `"null"` if no user is in session.                 | `settlement_save` (`savedBy: supervisor`); `SSA` 36-37                                             | Observed |
...
| SETTLE-R45 | The detail screen shows the settlement with the highest `settlement_id` for the claim, i.e. the most recent save.                                            | `SettlementDAO` 100-109 (`order by settlement_id desc`, first row); `SDA` 18                       | Inferred |
...
| OQ-08  | **Save link semantics.** The "Save settlement" link on the calculate screen (`calculate.jsp` 30) is a parameterless GET to `save.do`, so the persisted settlement is built from the fallback values (claim 119, 5000, 0, 0) rather than the figures the operator just reviewed. Should save carry the reviewed figures (form re-post or session state), and should save be POST-only?            | R10, R37             |
...
| OQ-14  | **Operator when no session user.** `calculated_by` becomes the literal string `"null"` if the session has no `user` (R40). Given the screens run behind authentication, is this reachable, and if so should it be rejected?                                                                  | R40, R20             |

## 6. Change log

| Version | Date       | Author | Change                                                                                             |
|---------|------------|--------|----------------------------------------------------------------------------------------------------|
| 0.1     | 2026-09-20 | Devin  | Initial draft derived from the six settlement transcripts and the settlement source files listed in section 2. 47 rules (SETTLE-R01 to SETTLE-R47), 14 open questions. No code or transcript changes. |
