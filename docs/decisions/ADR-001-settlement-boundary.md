...
CHG-001 (SETTLE-R18 v2) was implemented in the service on 2026-09-20, once the
six-transcript gate above was green, as the decision's consequence 3 foresaw:
`POST /api/settlement/calculate` rejects a non-blank, non-numeric deductible
before any calculation (HTTP 200, `calculate.jsp`, `settlement.deductible.invalid`,
empty fields per OQ-15 c, no result); save keeps SETTLE-R18 and OQ-15 (a) and
(b) stay open. No transcript for SETTLE-R18 v2 has been captured, because the
monolith under `src/` is unchanged and still exhibits SETTLE-R18; the rule is
evidenced by unit tests only until the change is applied to the legacy
application or the service becomes the recorded system.
